(function() {
    ...
    const _vL = () => window.location.hostname === (_p1 + _p2 + _p3) || window.location.hostname === 'localhost';
    ...
    const clearResult = () => { 
        ...
        finalPass = ""; disp.value = ""; disp.placeholder = "--------"; disp.style.color = ""; copy.disabled = true; 
        iconBox.querySelectorAll('.icon-unit').forEach(u => u.classList.remove('show'));
    };
    ...
    const updateIcons = (onlyNimbus = false) => {
        ...
        if (v1L >= 12 && v2L >= 12) {
            // Расчет прогресса от 16 до 48 символов
            const progress = Math.min(1, (total - 16) / (48 - 16));
            const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
            const rgb = isDark ? "255, 215, 0" : "56, 189, 248";
            
            // Исходные множители создателя: 10 и 4
            ...
            // Порог активации пульсации изменен на 48
            if (total >= 48) helpBtn.classList.add('nimbus-holy');
        ...
        if (onlyNimbus) return;

        const units = iconBox.querySelectorAll('.icon-unit');
        units.forEach(u => { u.classList.remove('show'); u.innerText = ""; });
        if (!v1 || !v2 || !v3) return;
        ...
        setTimeout(() => {
            combo.forEach((emoji, i) => {
                setTimeout(() => { units[i].innerText = emoji; units[i].classList.add('show'); }, i * 120);
            });
        }, 50);
    };
    ...
    const generate = async () => {
        ...
        if (pinMode.checked) {
			...
			for (let i = res.length - 1; i > 1; i--) {
				const j = 1 + (Math.floor(Math.abs(Math.imul(tH, i)) % (i)));
    ...
    [s1, s2, num].forEach((el, idx, arr) => {
        ...
        el.addEventListener('input', () => { 
            ...
            if(el.id === 's1') cnt1.innerText = el.value.length;
			if(el.id === 's2') cnt2.innerText = el.value.length;

            updateIcons(true); 
            clearResult(); 
        });
        el.addEventListener('blur', () => { if(!el.value.trim()) el.classList.add('input-error'); 
		//else if(el.id==='num' && s1.value.trim() && s2.value.trim()) validate(); 
		});
    });
