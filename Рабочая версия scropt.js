(function() {
    ...
    // ИСПРАВЛЕННАЯ функция проверки координат (добавлен индекс [0])
    ...
    // Кнопка ГЕНЕРИРОВАТЬ
    const handleGenStart = (e) => { 
        if(!isInsideBtn(e, genBtn)) return; 
    ...
    inputs.forEach((el, idx) => {
        ...
        el.addEventListener('input', () => { el.classList.remove('input-error'); if(el.id==='num') el.value=el.value.replace(/[^0-9]/g,'').slice(0,3); updateIcons(); clearResult(); });
    ...
    ['mousedown', 'keydown', 'touchstart'].forEach(ev => document.addEventListener(ev, () => { clearTimeout(panicTimer); panicTimer = setTimeout(() => resetAll(true), 60000); }));
